...
#saves geoid in county
for (i in 1:dim(lat_longs)[1]) {

  lat_longs[i,9] <- call_geolocator_latlon(lat = lat_longs$lat[i], lon = lat_longs$lon[i], benchmark = 2010,  vintage = 2010) #benchmark is when data was collected 
  print(i)

}


#write_csv(lat_longs, "data/LatLon_to_Tract_GEOIDinCounty_2010Attempt.csv")
geoid <- lat_longs
geoid <- filter(geoid_full, !is.na(county))



#geoid_full <- read_csv("data/LatLon_to_Tract_GEOIDinCounty_2010Attempt.csv") 
...
for (i in 1:dim(lat_longs_tract)[1]){
  lat_longs_tract$tract[i] <- str_sub(geoid$county[i],1,11)
  lat_longs_tract$CountyCode[i] <- as.numeric(str_sub(geoid$county[i],1,5))
}
...
completeData <- init_data %>%
  left_join(combinedVars, by = c("prolific_ID" = "prolific_ID", "time" = "time"))
